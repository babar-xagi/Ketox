...
Status: Phase 4 development contract. The backend is Kotlin/JVM through JNI.

## Build and call paths

```text
Rust source with #[kotlin_export], #[kotlin_class], #[kotlin_constructor], #[kotlin_enum], #[kotlin_model]
    -> shared signature/type validation in ketox-core
    -> metadata module (schema version 4)
    -> ketox-codegen
        -> ketox_jni.rs (included by the Rust library)
        -> RustApi.kt (compiled by Kotlin, contains object RustApi, classes, enums, models)
        -> ketox-metadata.json

Kotlin object / class method / value types
    ...
    -> the original Rust function, method, or constructor
```
...
`ketox-core` depends on syntax and serialization libraries, not JNI. It validates exported signatures, computes Kotlin names and JVM type descriptors, and produces portable metadata. `ketox-codegen` turns valid metadata into inspectable Rust, Kotlin, and JSON. `ketox-jni` owns JNI conversion, the panic/exception boundary, array/string conversions, and the global thread-safe handle registry for stateful objects. `ketox-cli` exposes the same discovery and generation path through `ketox inspect` and `ketox generate`.
...
Metadata uses explicit schema version 4 (supporting versions 1, 2, 3, and 4). Unknown fields and unsupported versions are rejected; semantic validation runs before generation. Source-discovered functions, classes, enums, and models are deterministically ordered by their Rust names.
